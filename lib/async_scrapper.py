...
from lib.imdb import Imdb, ImdbMovie, ImdbSerie
...
def get_details(_media_data):
    ...
    if "TVEpisode" in media_type:
        logger.info(f"{imdb_id}: {media_type} Skipped")
        return False
...
def process(raw_urls):
    ...
    urls = (url for url in raw_urls if not check_item_exists(get_imdb_id(url)))
    if not urls:
        return None
    start_media = time.time()
    logger.info(f"start_media {(time.time() - start_media)} seconds ---")
    _media_infos = get_media_data(urls)

    del urls
    gc.collect()

    logger.info(f"end media {(time.time() - start_media)} seconds ---")
